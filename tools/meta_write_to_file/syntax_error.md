...
11. Type annotations using `( expr : Type)` syntax or `let var: Type = expr` syntax
...
12. Type associated method call using `TypePath::method()` syntax
...
- ✅ Correct:
  ...
  ```
  Only type path is supported before `::method()`, it is not allowed to have generic parameters there.
